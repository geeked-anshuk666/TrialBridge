...
Core registry discovery does not require `GROQ_API_KEY`. Without it, TrialBridge uses deterministic profile keywords and safe, registry-derived explanations; Groq only improves localization and plain-language explanations. Render can use `/health` for liveness and `/ready` for Neon connectivity.
